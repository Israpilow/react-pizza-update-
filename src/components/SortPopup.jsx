import React from 'react';
import { useDispatch } from 'react-redux';

import { setSortBy } from '../redux/actions/filters';

function SortPopup() {
  const sortItems = [
    { name: 'Популярные (DESC)', type: 'rating', order: 'desc' },
    { name: 'Популярные (ASC)', type: 'rating', order: 'asc' },
    { name: 'По убыванию цены (DESC)', type: 'price', order: 'desc' },
    { name: 'По возрастанию цены (ASC)', type: 'price', order: 'asc' },
    { name: 'По алфавиту (DESC)', type: 'name', order: 'desc' },
    { name: 'По алфавиту (ASC)', type: 'name', order: 'asc' },
  ];
  const dispatch = useDispatch();
  const sortItem = Object.values(sortItems[0]);

  const [sortActive, setSortActive] = React.useState(sortItem[0]);
  const [sortPopup, setSortPopup] = React.useState(false);
  const sortRef = React.useRef();

  const onSortClickActive = (obj) => {
    setSortActive(obj.name);
    setSortPopup(false);
    dispatch(setSortBy(obj));
  };

  const onPopupSortClick = () => {
    setSortPopup(!sortPopup);
  };

  const onHandleClickPopup = (event) => {
    if (!event.path.includes(sortRef.current)) {
      setSortPopup(false);
    }
  };

  React.useEffect(() => {
    document.body.addEventListener('click', onHandleClickPopup);
  }, []);

  return (
    <div ref={sortRef} className="sort">
      <div className="sort__label">
        <svg
          width="10"
          height="6"
          viewBox="0 0 10 6"
          fill="none"
          xmlns="http://www.w3.org/2000/svg">
          <path
            d="M10 5C10 5.16927 9.93815 5.31576 9.81445 5.43945C9.69075 5.56315 9.54427 5.625 9.375 5.625H0.625C0.455729 5.625 0.309245 5.56315 0.185547 5.43945C0.061849 5.31576 0 5.16927 0 5C0 4.83073 0.061849 4.68424 0.185547 4.56055L4.56055 0.185547C4.68424 0.061849 4.83073 0 5 0C5.16927 0 5.31576 0.061849 5.43945 0.185547L9.81445 4.56055C9.93815 4.68424 10 4.83073 10 5Z"
            fill="#2C2C2C"></path>
        </svg>
        <b>Сортировка по:</b>
        <span onClick={onPopupSortClick}>{sortActive}</span>
      </div>
      {sortPopup && (
        <div className="sort__popup">
          <ul>
            {sortItems.map((obj, index) => {
              return (
                <li
                  key={index}
                  onClick={() => onSortClickActive(obj)}
                  className={sortActive === obj.name ? 'active' : ''}>
                  {obj.name}
                </li>
              );
            })}
          </ul>
        </div>
      )}
    </div>
  );
}

export default SortPopup;
